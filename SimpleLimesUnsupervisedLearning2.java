		 ...
		 // private static String endpoint = "http://dbtune.org/musicbrainz/sparql";
		  ...
		  String QSrc = "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#>"
		  		+ "SELECT DISTINCT ?p where \r\n" + 
			  		"{\r\n" + 
			  		"?s ?p ?o {select ?s where {?s a <http://purl.org/ontology/mo/MusicArtist> } Limit 100}\r\n" + 
	        ...
	        if (conf.getMlAlgorithmName().equals("WOMBAT SIMPLE")) {
	            try {
	                results = MLPipeline.execute(sourceCache, 
	                		                     targetCache, 
	                		                     conf, 
	                		                     conf.getMlAlgorithmName(), //WOMBAT SIMPLE		
	                		                     conf.getMlImplementationType(), //UNSUPERVISED
	                		                     conf.getMlAlgorithmParameters(), //[max execution time in minutes : 60]
	                		                     conf.getTrainingDataFile(),  // Null
	                		                     conf.getMlPseudoFMeasure(), // Null
