...
public class SourceClassTargetMapping {
	...
	public static void main(String[] args) throws UnsupportedMLImplementationException {
		...
		String fullClassName = SourceClassTargetMapping.class.getName();
		...
		while(classes_sc.hasNext())
		 {
			...
			ParameterizedSparqlString pss_sc = new ParameterizedSparqlString("select ?s where {?s a ?class } Limit 100");
			//Resource classvar = ResourceFactory.createResource(musicclass.get("?class").toString()
				//	);
			...
			while(instances_sc.hasNext())
			{
				...
				ParameterizedSparqlString pss1_sc = new ParameterizedSparqlString("SELECT ?p ?o where \r\n" + 
																					"{\r\n" + 
																					"\r\n" + 
																					"?s ?p ?o \r\n" + 
																					"\r\n" + 
			...
			/*----------------------Begining of Target -------------------------------------------- */
				...
				while(classes_tc.hasNext()) 
				{
					...
					while(instances_tc.hasNext()) 
					{
						...
						ParameterizedSparqlString pss1_tc = new ParameterizedSparqlString("SELECT ?p ?o where \r\n" + 
																							"{\r\n" + 
																							"\r\n" + 
																							"?s ?p ?o \r\n" + 
																							"\r\n" + 
					  ...
					  while(properties_tc.hasNext()) 
					  {
						  ...
						  //ACache sourceCache = null;
						  //ACache targetCache =null;
						  HybridCache sourceCache = new HybridCache();
						  HybridCache targetCache = new HybridCache();
						  //ACache sourceCache = (ACache) musicproperties_sc.get("?o");
					     //ACache targetCache = (ACache) musicproperties_tc.get("?o");
						 // System.out.println("s-"+musicinstance_sc.get("?s").toString()+"p-"+musicproperties_sc.get("?p").toString()+"o-"+musicproperties_sc.get("?o").toString() );
			    	      ...
			    	      if (sim > 100) 
			    	       {
							...
							//System.out.println(property.get("?p")+ "========" +property.get("?o")+"classes  "+musicclass.get("?class"));
