...
public class SimpleLimesUnsupervisedLearning2 {

	private static final String SYSTEM_DIR = System.getProperty("user.dir");

	public static void main(String[] args) {

		String fullClassName = SimpleLimesUnsupervisedLearning2.class.getName();
		...
		prop_ListX =new ArrayList<String>(Arrays.asList("<http://purl.org/ontology/mo/musicbrainz>","<http://www.w3.org/2002/07/owl#sameAs>","<http://www.w3.org/2000/01/rdf-schema#label>"));	  
		...
		/*  String QSrc = "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#>"
		  		+ "SELECT DISTINCT ?p where \r\n" + 
			  		"{\r\n" + 
			  		"?s ?p ?o {select ?s where {?s a <http://purl.org/ontology/mo/MusicArtist> } Limit 1}\r\n" + 
		...
		/*  String QTrg = "SELECT DISTINCT ?p where \r\n" + 
		  ...
		  String QTrg33 ="PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\r\n" + 
		  		"select  distinct ?p where {\r\n" + 
		  		" {?s ?p ?o\r\n" + 
		  		" filter( !strstarts( str(?p), str(rdf:) ) )} \r\n" + 
		  		"} LIMIT 10";*/
		...
		// Scache and Tcache
		ACache sourceCache = HybridCache.getData(conf.getSourceInfo());
		ACache targetCache = HybridCache.getData(conf.getTargetInfo());

		// Preprocessing Scache and Tcache
		sourceCache = Preprocessor.applyFunctionsToCache(sourceCache, conf.getSourceInfo().getFunctions());
		System.out.println("zzzzz"+sourceCache.getAllInstances().get(0));
		targetCache = Preprocessor.applyFunctionsToCache(targetCache, conf.getTargetInfo().getFunctions());		  

		//Wombat 

		if (conf.getMlAlgorithmName().equals("WOMBAT SIMPLE")) {
			try {
				/* results = MLPipeline.execute(sourceCache, 
	                		                     targetCache, 
	                		                     conf, 
	                		                     "WOMBAT SIMPLE",//conf.getMlAlgorithmName(), //WOMBAT SIMPLE		
	                		                     MLImplementationType.UNSUPERVISED,//conf.getMlImplementationType(), //UNSUPERVISED
	                		                     mlParameters,//conf.getMlAlgorithmParameters(), //[max execution time in minutes : 60]
	                		                     null,//conf.getTrainingDataFile(),  // 
	                		                     null,//conf.getMlPseudoFMeasure(), // Null EvaluatorType.
	                		                     MAX_ITERATIONS_NUMBER);*/
				//String mlAlgorithmName="WOMBAT_SIMPLE";
				Class<? extends ACoreMLAlgorithm> clazz = MLAlgorithmFactory.getAlgorithmType("wombat simple");
				MLResults mlm;
				UnsupervisedMLAlgorithm mlu = new UnsupervisedMLAlgorithm(MLAlgorithmFactory.getAlgorithmType("wombat simple"));
				mlu.init(mlParameters, sourceCache, targetCache);
				System.out.println("mlu   1 >>"+mlu);
				mlu.getMl().setConfiguration(conf);
				System.out.println("mlu   2  >>>>>"+mlu);
			...
			System.out.println("results>>>"+results.getNumberofMappings());
		} 
